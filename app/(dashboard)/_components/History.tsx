...
import {
  GetHistory,
  GetHistoryDataResponseType,
} from "@/actions/GetHistoryData";
...
const History = ({ userSettings }: { userSettings: UserSettings }) => {
  ...
  const historyDataQuery = useQuery<GetHistoryDataResponseType>({
    ...
    queryFn: () => GetHistory(timeFrame, period.year, period.month),
  });
  const dataAvailable =
            ...
            {!dataAvailable && (
              <Card className="flex h-[300px] flex-col items-center justify-center bg-background">
                ...
                <p className="text-sm text-muted-foreground">
                  Try selecting a different period of adding new transactions
                </p>
              </Card>
            )}
          </SkeletonWrapper>
...
function CustomTooltip ({active , payload , formatter} : any){
  ...
  return <div className="min-w-[300px] rounded border bg-background p-4">
    ...
    <ToolTipRow formatter={formatter} label="Income" value={income} bgColor="bg-green-500" textColor="text-green-500" />
    <ToolTipRow formatter={formatter} label="Balance" value={income - expense} bgColor="bg-gray-100" textColor="text-foreground" />
  </div>
}

function ToolTipRow({label , formatter , value , bgColor , textColor} :{
  label : string,
    ...
    <div className="flex items-center gap-2">
      ...
      <div className="flex w-full justify-between">
        ...
        <div className={cn("text-sm font-bold" , textColor)}>
          <CountUp start={0} end={value} duration={0.5} decimal="0" formattingFn={formattingFn} />
        </div>
      </div>
    </div>
  )
